        ...
        private List<Message> GetMessagesForStatusAndController(int? statusId, int? controllerId, int ticketId)
        {
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                if (statusId == 1)
                {
                    ...
                    {
                        messages.Add(new Message { UserId = l2AdminUserId, Text = $"New ticket is generated by {controllerName}" });
                    }
                    // Send a notification to the manager
                    if (managerId.HasValue)
                    {
                        messages.Add(new Message { UserId = managerId.Value, Text = $"New ticket created by your employee: {ticket.UserId}" });
                    }
                }
